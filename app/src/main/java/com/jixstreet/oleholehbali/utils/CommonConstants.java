package com.jixstreet.oleholehbali.utils;

/**
 * Created by satryaway on 12/6/2015.
 */
public class CommonConstants {
    public static final String LOCAL_SERVER = "http://192.168.1.1/";
    public static final String BASE_URL = LOCAL_SERVER + "oleholehbali/";
    public static final String SERVICE_GET_PRODUCT_BY_CATEGORY = BASE_URL+ "product/c/";
    public static final String SERVICE_GET_PRODUCT_IMAGE = BASE_URL+ "assets/images/";
    public static final String SERVICE_POST_TRANSACTION = BASE_URL+ "transaction";
    public static final String COD = "Cash on Delivery";
    public static final String VIA_TRANSFER = "Via Transfer";
    public static final String VIA_CC = "Via Credit Card";
    public static String CATEGORY = "category";
    public static String CULINARY = "culinary";
    public static String SOUVENIR = "souvenir";
    public static String STATUS = "status";
    public static int RESULT_OK_CODE = 1;
    public static String MESSAGE = "message";
    public static String RETURN_DATA = "return_data";
    public static String ID = "id";
    public static String NAME = "name";
    public static String DESCRIPTION = "description";
    public static String IMAGE = "image";
    public static String PRODUCT = "product";
    public static String SPECIFICATION = "specification";
    public static String PRICE = "price";
    public static String STOCK = "stock";
    public static String OOB_APP = "Oleh-Oleh Bali Application";
    public static String IS_LOGGED_IN = "is_logged_in";
    public static String PRODUCTS = "products";
    public static int PRODUCT_CODE = 27;
    public static String TRANSACTION = "transaction";
    public static String PAYMENT_METHOD = "payment_method";
    public static String EMAIL = "email";
    public static String PHONE = "phone";
    public static String ADDRESS = "address";
    public static String KEC = "kec";
    public static String KAB = "kab";
    public static String PROV = "prov";
    public static String POSTAL_CODE = "postal_code";
    public static String TOTAL = "total";
    public static String PRODUCT_ARRAY = "product_id[]";
    public static String QTY_ARRAY = "qty[]";
    public static String OK = "OK";
    public static String PRODUCT_ID = "product_id";
    public static String QTY = "qty";
}
